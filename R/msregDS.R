...
msregDS <- function(formula, df_cohort, df_survey,
  ...
  # evaluate input objects
  df_cohort <- eval(parse(text=df_cohort), envir = parent.frame())
  df_survey <- eval(parse(text=df_survey), envir = parent.frame())
  
  # parse data
  # Ensure the cohort and survey data are a data frame
  if (!is.data.frame(df_cohort)) stop("Cohort data must be a data.frame!", call.=FALSE)
  if (!is.data.frame(df_survey)) stop("Survey data must be a data.frame!", call.=FALSE)
  ...
  formula <- gsub("vertical_line", "|", formula, fixed = TRUE)
  ...
  parsed_formula <- Formula::Formula(formula)
  ...
  if (length(check_vars_main) > 0) {
    stop(sprintf("The following variables are missing in the data frame: %s", paste(check_vars_main, collapse = ", ")), call.=FALSE)
  }
  check_vars_matching <- setdiff(matching_model, names(df_survey))
  if (length(check_vars_matching) > 0) {
    stop(sprintf("The following variables are missing in the data frame: %s", paste(check_vars_matching, collapse = ", ")), call.=FALSE)
  }
